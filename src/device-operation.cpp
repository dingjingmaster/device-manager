...
void DeviceOperation::udiskFormat(QString type, QString labelName)
{
    ...
    udisks_block_call_format(mDiskBlock, type.toLower().toUtf8().constData(), g_variant_builder_end(&optionsBuilder), &mFormatCancel, GAsyncReadyCallback(formatCB), this);
}
...
void DeviceOperation::udiskRepair()
{
    ...
    udisks_filesystem_call_repair(mDiskFilesystem, g_variant_builder_end(&optionsBuilder), &mRepairCancel, GAsyncReadyCallback(repairCB), this);
}
